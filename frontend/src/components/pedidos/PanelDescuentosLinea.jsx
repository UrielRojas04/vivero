...
const PanelDescuentosLinea = ({ descuentos, error, onAgregar, onQuitar, onActualizar, onRecargarDefaultsProveedor }) => (
  <div className="pt-2 pb-3 px-3 bg-paper border border-line-strong rounded-panel shadow-lg">
    <div className="flex items-center justify-between">
      <label className="block text-[11px] font-semibold text-muted uppercase tracking-wider">Descuentos pactados</label>
      <div className="flex items-center gap-2.5">
        <button
          ...
          className="flex items-center gap-0.5 text-[11px] font-semibold text-muted hover:text-body cursor-pointer"
        >
        ...
        <button
          type="button"
          onClick={onAgregar}
          className="flex items-center gap-0.5 text-[11px] font-semibold text-accent hover:text-accent-ink cursor-pointer"
        >
    ...
    {descuentos.length === 0 ? (
      <p className="mt-1 text-[11px] text-faint italic">Sin descuentos cargados.</p>
    ) : (
        ...
        {descuentos.map((d, index) => (
          <div key={index} className="flex items-center gap-1.5">
            <input
              ...
              className="flex-1 min-w-0 px-2 py-1 rounded-base border border-line bg-paper text-xs focus:outline-none focus:ring-2 focus:ring-accent"
            />
            <div className="w-16 shrink-0">
              <FormattedNumberInput
                ...
                placeholder="%"
                className="w-full px-2 py-1 rounded-base border border-line bg-paper text-xs text-right focus:outline-none focus:ring-2 focus:ring-accent font-mono tabular-nums"
              />
            </div>
            <button
              type="button"
              onClick={() => onQuitar(index)}
              className="p-1 rounded-base text-danger hover:bg-danger-bg transition-colors cursor-pointer shrink-0"
    ...
    {error && <p className="mt-1 text-[11px] text-danger leading-tight">{error}</p>}
  </div>
);
