> ## 🔴 ARCHIVADO COMO SUPERADO/ABANDONADO (2026-08-26)
>
> **Este change NO se completó como estaba diseñado y no se va a retomar.** Se archiva a las 20/61 tareas con las que quedó pausado desde el addendum del 2026-08-22 (ver abajo), no como "implementado con éxito".
>
> **Por qué se abandona:** el problema real que este change buscaba resolver —que la ficha del producto (`Producto.costoProducto`/`precio`) puede quedar desactualizada respecto del último costo real pagado, sin que nadie lo note— ya está cubierto por dos mecanismos construidos DESPUÉS, en esta misma sesión, con un enfoque distinto y más simple:
> 1. **El ratchet automático de `ProductoServiceImpl.actualizarFichaSiCostoFinalSupera`** (fix del 2026-08-26, ver Javadoc de `ProductoService.actualizarFichaSiCostoFinalSupera`): compara el costo FINAL completo (base → descuentos → IVA → envío) en cada pedido confirmado contra el final actual de la ficha, y actualiza `costoProducto`/descuentos/IVA/envío/`precio` sola cuando el pactado es estrictamente mayor. No requiere que el jefe entre a revisar nada: el catálogo se mantiene solo.
> 2. **El badge de margen en `frontend/src/pages/Productos.jsx`** ("Vendiendo a pérdida" / "Margen reducido"), del change `pedido-grilla-visual`: hace visible en la propia grilla cuándo el precio de venta quedó corto respecto del costo real, sin necesitar un panel separado.
>
> Entre los dos, el trabajo manual de "acordarse de ir a mirar Productos" que este change buscaba eliminar (ver la historia del producto `id=31` más abajo) ya no existe: el ratchet lo resuelve solo en el caso más común (costo subió), y el badge avisa en el caso que el ratchet no cubre (venta a pérdida visible al toque). Un panel de revisión manual dedicado quedó redundante.
>
> **Además, y ya no era sólo cuestión de rediseño:** el addendum del 2026-08-22 (abajo) señaló que el criterio de detección de este panel (comparar `costo_producto` contra el `costo_base` del **último `INGRESO`/`AJUSTE_INICIAL`**) quedó obsoleto en cuanto `costeo-fifo-herramientas` activa `costeoPorCapasHabilitado=true`: el costo de referencia pasa a ser el máximo entre las capas activas, no el último movimiento cronológico. Ese rediseño nunca se hizo, y con el ratchet y el badge ya cubriendo el problema, no hace falta hacerlo.
>
> **Código removido de este mismo commit:** el código backend parcial que sí llegó a implementarse (grupos 1, 2, 3 y 6 de `tasks.md` — la query de detección, el DTO, el endpoint `GET /api/productos/revision-costos` y la columna `movimiento_revision_descartado_id`) se sacó del repo. Se verificó con `grep` que el frontend nunca llegó a consumirlo (ningún archivo bajo `frontend/src/` referencia `revision-costos` ni `RevisionCosto`), y que mantenerlo no valía la pena: con el flag `costeoPorCapasHabilitado=true` ya activo en Herramientas, su criterio de detección daría resultados incorrectos si alguien lo llegara a usar. Se borraron: `dto/RevisionCostoProductoDTO.java`, `repositories/RevisionCostoProjection.java` completos; el endpoint en `ProductoController`; la query nativa `findRevisionCostos`/`findProductosUsdSinConversionRegistrada` en `ProductoRepository`; el campo `movimientoRevisionDescartadoId` (+ getter/setter) en `Producto.java`; el método `listarRevisionCostos` (+ helper `armarFilaRevisionCosto`) en `ProductoService`/`ProductoServiceImpl`. La columna `productos.movimiento_revision_descartado_id` también se borró de la base real (`ALTER TABLE productos DROP COLUMN`), porque `ddl-auto=update` nunca la hubiera borrado sola.
>
> **Las specs delta de este change (`specs/frontend-productos/spec.md`, `specs/revision-costos-productos/spec.md`) NO se sincronizan a `openspec/specs/`** al archivar: describen un panel que nunca se construyó, así que aplicarlas dejaría la spec principal mintiendo sobre una capability inexistente. `openspec/specs/revision-costos-productos/` no llega a crearse; `openspec/specs/frontend-productos/spec.md` queda tal cual estaba, sin los cuatro requisitos nuevos que este change iba a agregarle.
>
> ---
>
