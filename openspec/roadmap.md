...
>
> **`registro-semillas-clientes` (implementado 2026-09-02/03, pendiente de archivar — `openspec/changes/registro-semillas-clientes/`):** Pantalla nueva (`/registro-semillas`, grupo Catálogo del sidebar, unidad Vivero) para que el jefe registre digitalmente las semillas que los clientes traen al invernadero, reemplazando la libreta en papel. Entidad nueva `RegistroSemilla` (tabla `registros_semillas`), **independiente de `Siembra`**: es sólo constancia de recepción, no crea `Producto` ni `MovimientoStock`. Cliente vinculado opcional con snapshot de nombre/teléfono (sobrevive al borrado lógico del cliente). Cantidad estructurada como `cantidad + unidadCantidad (SEMILLAS|SOBRES|GRAMOS)` con `contenidoPorSobre` opcional; `totalSemillas` es derivado (nunca persistido). `lote` es texto libre sin unicidad, mismo precedente que `Siembra.codigoLote`. Boletita de comprobante (`ComprobanteSemillaModal.jsx`, PDF/PNG/WhatsApp) reutilizando el mecanismo de `ComprobanteVentaModal.jsx`: los helpers compartidos (`normalizarTelefonoWhatsApp`, `generarPngDeNodo`, flags de dispositivo táctil, `abrirWhatsApp` con la variable de módulo `ventanaWhatsAppAbierta` para reutilizar la misma pestaña) se extrajeron a `frontend/src/utils/comprobanteExport.js`, y `ComprobanteVentaModal.jsx` (remito de venta en producción) se migró a consumirlos sin cambios de comportamiento. 15 tests de backend nuevos contra Postgres real (sin mocks), verdes. **Pendiente antes de probar en el navegador:** reconstruir el contenedor `vivero-backend` y verificación manual de la task 5.3 (regresión del remito de venta).
>
> **Actualización 2026-09-03 (pedido explícito del dueño, amplía el alcance — grupo 10 de `tasks.md`):** los permisos de este change dejaron de reutilizar `LEER_SIEMBRAS`/`ESCRIBIR_SIEMBRAS`. **`PermisoEnum` SÍ se tocó**: se agregaron `LEER_REGISTRO_SEMILLAS(19L)`/`ESCRIBIR_REGISTRO_SEMILLAS(20L)` al final del enum, para poder otorgar acceso a Registro de Semillas sin otorgar Siembras y viceversa. `DataInitializer` **no** necesitó cambios (JEFE/COLEGA los reciben solos vía `EnumSet.allOf`). De paso se corrigió un bug de seguridad real en `SiembraController`: ningún método de escritura exigía `ESCRIBIR_SIEMBRAS`, así que un usuario de solo lectura podía crear/editar/borrar/finalizar siembras. También se reescribió el filtrado por unidad de negocio de `UsuariosAdmin.jsx` (roles y permisos): condicionales ad-hoc reemplazados por un campo `unidades: [...]` explícito por sección (mismo vocabulario que `navGroups`), se sacaron las casillas de Finanzas/Cheques de las 3 unidades (acceso reservado a los administradores de cada unidad, ausencia deliberada) y `ADMIN_SIEMBRAS` de la casilla "Siembras" (permiso que ningún `@PreAuthorize` del backend verifica), y la pestaña "Avanzado (Permisos)" también quedó filtrada por unidad (con `LEER_FINANZAS` oculto siempre). 8 tests de backend nuevos (`SiembraControllerPermisoTest`, `RegistroSemillaControllerPermisoTest`) contra Postgres real, con evidencia RED/GREEN genuina (revirtiendo temporalmente el fix para confirmar el fallo antes de reaplicarlo).
>
